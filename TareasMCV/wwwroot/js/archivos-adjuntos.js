...
function prepararArchivosAdjuntos(archivosAdjuntos) {
    archivosAdjuntos.forEach(archivosAdjunto => {
        let fechaCreacion = archivosAdjunto.fechaCreacion;
        if (archivosAdjunto.fechaCreacion.indexOf('Z') === -1) {
            fechaCreacion += 'Z';
        }

        const fechaCreationDT = new Date(fechaCreacion);
        archivosAdjunto.publicado = fechaCreationDT.toLocaleString();

        tareaEditarVM.archivosAdjuntos.push(new archivoAdjuntoViewModel({ ...archivosAdjunto, modoEdicion: false }));
    });
}
...
$(function () {
    $("#reordenable-archivos-adjuntos").sortable({
        axis: 'y',
        stop: async function () {
